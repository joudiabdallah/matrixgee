...
NumericMatrix correlation_cols_calc_cpp(NumericVector residuals,
        ...
        //return identity matrix if the corstr_cols is independence
        ...
        //reshape the residuals vector into a matrix
        ...
        if (corstr_cols == "Exchangeable") {
                denominator = ((0.5 * sample_size * rows_no * cols_no * (cols_no - 1)) - parameters_no) * dispersion_parameter;
                        ...
                        for (int r = 0; r < rows_no; ++r) {
                                for (int j = 0; j < cols_no - 1; ++j) {
                                        for (int k = j + 1; k < cols_no; ++k) {
                                                total_numerator += residuals_matrix_all(r, start_col + j) *
                                                        residuals_matrix_all(r, start_col + k);
                                        }
                                }
                        }
                }
                ...
                denominator = ((sample_size * rows_no * (cols_no - 1)) - parameters_no) * dispersion_parameter;

                for (int i = 0; i < sample_size; ++i) {
                        int start_col = i * cols_no;
                        for (int r = 0; r < rows_no; ++r) {
                                for (int c = 0; c < cols_no - 1; ++c) {
                                        total_numerator += residuals_matrix_all(r, start_col + c) *
                                                residuals_matrix_all(r, start_col + c + 1);
                                }
                        }
                }
        }
